...
test.describe('docs registry coverage', () => {
  ...
  test('representative rich-example pages still render RadCN components', async ({ page }) => {
    ...
    await expect(paginationDemo.locator('[data-radcn-pagination-ellipsis] .sr-only')).toHaveText('More pages')
    ...
    await expect(collapsibleTrigger.locator('.sr-only')).toHaveText('Toggle')
    ...
    await expect(page.locator('[data-radcn-docs-dialog-family="dialog-close-button"] label[for="docs-dialog-share-link"]')).toHaveClass(/sr-only/)
