...
test('candidate command demo matches named upstream grouped command behavior', async ({ page }) => {
  let opened = await freshCommand(page, 'demo')
  await expect(opened.root).toHaveClass(/radcn-fixture-command-demo/)
  await expect(opened.input).toHaveAttribute('placeholder', 'Type a command or search...')
  await expect(page.locator('[data-radcn-command-group-heading]')).toHaveText(['Suggestions', 'Settings'])
  await expect(page.locator('[data-radcn-command-separator]')).toHaveCount(1)
  await expect(page.locator('[data-fixture-command-icon]')).toHaveCount(6)
  await expect(page.locator('[data-radcn-command-shortcut]')).toHaveText(['⌘P', '⌘B', '⌘S'])
  ...
  await expect(page.getByRole('option', { name: 'Calculator' })).toHaveAttribute('aria-disabled', 'true')
  await expect(page.getByRole('option', { name: /Profile/ })).toBeVisible()
  await expect(page.getByRole('option', { name: /Billing/ })).toBeVisible()
  await expect(page.getByRole('option', { name: /Settings/ })).toBeVisible()

  await opened.input.fill('bill')
  await expect(page.getByRole('option', { name: /Billing/ })).toBeVisible()
  await expect(page.getByRole('option', { name: 'Calendar' })).toBeHidden()
  await page.keyboard.press('Enter')
  await expect(opened.root).toHaveAttribute('data-value', 'billing')

  await opened.input.fill('zzzz')
  await expect(page.locator('[data-radcn-command-empty]')).toBeVisible()
})

test('candidate command dialog demo opens from app-owned shortcut', async ({ page }) => {
  await page.goto(`${candidate}/fixtures/command/dialog-demo`)
  let dialog = page.locator('.radcn-command-dialog[data-radcn-dialog-content]').first()
  await expect(page.locator('[data-fixture-command-dialog-guidance]')).toContainText('Press')
  await expect(page.locator('[data-radcn-kbd]')).toHaveText(['⌘', 'J'])
  ...
  await expect(page.locator('[data-radcn-command-group-heading]')).toHaveText(['Suggestions', 'Settings'])
  await expect(page.locator('[data-radcn-command-shortcut]')).toHaveText(['⌘P', '⌘B', '⌘S'])
  await expect(page.getByRole('option', { name: 'Calculator' })).not.toHaveAttribute('aria-disabled', 'true')
  await expect(page.getByRole('option', { name: /Settings/ })).toBeVisible()
