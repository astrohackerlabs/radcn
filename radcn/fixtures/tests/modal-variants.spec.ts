...
test('candidate alert dialog renders named demo parity', async ({ page }) => {
  await page.goto(`${candidate}/fixtures/alert-dialog/demo`)
  let root = page.locator('[data-radcn-alert-dialog]')
  let trigger = page.locator('[data-radcn-alert-dialog-trigger]')
  let portal = page.locator('[data-radcn-alert-dialog-portal]')
  let overlay = page.locator('[data-radcn-alert-dialog-overlay]')
  let content = page.locator('[data-radcn-alert-dialog-content]')

  await expect(root).toHaveAttribute('data-state', 'closed')
  await expect(trigger).toHaveText('Show Dialog')
  await expect(trigger).toHaveAttribute('aria-haspopup', 'dialog')
  await expect(trigger).toHaveClass(/radcn-alert-dialog-trigger/)
  await expect(trigger).toHaveClass(/radcn-button/)
  await expect(trigger).toHaveClass(/radcn-button--outline/)
  await expect(page.locator('[data-radcn-alert-dialog-media]')).toHaveCount(0)

  await trigger.click()
  await expect(root).toHaveAttribute('data-state', 'open')
  await expect(trigger).toHaveAttribute('data-state', 'open')
  await expect(portal).toHaveAttribute('data-state', 'open')
  ...
  await expect(page.locator('body')).toHaveCSS('overflow', 'hidden')
  await expect(page.locator('[data-radcn-portal-root] [data-radcn-alert-dialog-portal]')).toHaveCount(1)
  await expect(page.locator('[data-radcn-alert-dialog-title]')).toHaveText('Are you absolutely sure?')
  await expect(page.locator('[data-radcn-alert-dialog-description]')).toHaveText(
    'This action cannot be undone. This will permanently delete your account and remove your data from our servers.',
  )
  await expect(page.locator('[data-radcn-alert-dialog-footer]')).toBeVisible()
  await expect(page.locator('[data-radcn-alert-dialog-cancel]')).toHaveText('Cancel')
  await expect(page.locator('[data-radcn-alert-dialog-action]')).toHaveText('Continue')
